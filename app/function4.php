...
<li class="list-group-item">
  ...
  echo '  <form method="POST" action="">
            ...
            if ($row->payment_status =='pending' && ($row->expiringTime < $timeNow || $row->expiringTime == 'NULL') ){
...
function ActivationReceiverView($user_id){
  $settings = DB::table('settings')->where('id', 1)->first();
  $users = DB::table('activationFee')->where('receiver_id', $user_id)->take(1)->orderBy('id', 'DESC')->get();
