...
//$CheckRequest = DB::table('requestmaching')->where('userid', '=', $user_id)->count();
...
if ($CheckRequest > 0) {
 ...
 You already have active or pending package with us, not allowed
    ...
    elseif (isset($chek2) && $Amount < $chek2->amount) {
        echo '<div class="alert alert-danger" role="alert">
 You cannot Choose a lower Package
</div>';
    }
else{
  ...
  if (empty($firstname) || empty($lastname) || empty($phonenumber) || empty($bankname) || empty($accountnumber) || empty($accountname) || empty($accounttype) || empty($country) || empty($state)) {
    ...
    if ($receiver != 1) {
     $receeiversNew = $receiver;
      ...
      $settings = DB::table('settings')->where('id', 1)->first();
      $startDate = time();
      $timeNow = date('Y-m-d H:i:s', strtotime('+'.$settings->activationFeeExp.'day', $startDate));
     $random = DB::table('activationFee')->insert(
      ...
      $message = urlencode("Hi, ".$firstname."  You have successfully registered to ngHELPERS, you have a pending activation fee. check your dashboard to proceed. https://bit.ly/2FDnK5U");
      $sender= urlencode("ngHELPERS");
      $mobile = $phonenumber;
 ...
 $sql = DB::table('marching')
      ...
      $TheBalance = DB::table('bank')->where('userid', $user_id)->first();
       ...
       if ($getRef >=1) {
        ...
        if ($getRefSpensor->status ==1) { //if it gets the sponsor of the sender
       ...
       $new_amount = ($percentage / 100) * $totalWidth;
        ...
        if ($getRefSpensor->parent == $getRefSpensor->sponsor) //if the guider is same as the sponsor
        {

            $cbalance =   $BankSpensor->balance + $new_amountP + $new_amount;

            DB::table('bank') //update guider balance to recom balance
            ->where('userid', $getRefSpensor->parent)
                ->update(array('balance' => $cbalance));
        }else {
            ...
            DB::table('bank')//update user balance to recom balance
            ->where('userid', $getRefSpensor->sponsor)
                ->update(array('balance' => $balance));
        }

            $getC = DB::table('marching')->where('sender_id', $user_id)->where('payment_status', 'pending')->orwhere('payment_status', 'waiting')->count();
        if (isset($getC) && $getC < 2) {
            DB::table('referral')
                ->where('userid', $sender_userid)
...
function recomitmentWithdraw($user_id){
   ...
   if ($id) {
    $TheBalance = DB::table('bank')->where('userid', $user_id)->first();
    //even number depicts recommitment
...
$startDate = time();
$timeX        =   date('Y-m-d H:i:s', strtotime('+'.$settings->timeMargin.' day', $startDate));
$user = DB::table('users')->where('id', $user_id)->first();
