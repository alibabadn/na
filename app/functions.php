...
{
$settings = DB::table('settings')->where('id', 1)->first();
	$user_request = DB::table('requestmaching')->where('userid', $user_id)->take(1)->orderBy('id', 'DESC')->get();
		...
		echo ' <div class="col-lg-12">
                  <div class="work-amount card"><i class="fa fa-bolt" style="color: green; font-size: 35px;margin-top: -10px;"></i>
                    ...
                    <?php if ($row->status == "active") {
                     echo '<center style="margin-left: -20px;margin-right: -20px;margin-bottom: -20px;border-radius: 0px !important;"><button class="btn btn-success btn-lg btn-block" style="border-radius: 0px !important;">Approved to Pay</button></center>';
                     }
...
{
  $settings = DB::table('settings')->where('id', 1)->first();
	$user_request = DB::table('marching')->where('sender_id', $user_id)->take(3)->orderBy('id', 'DESC')->get();
	if ($user_request >0) {
		foreach ($user_request as $row) {
...
{
$settings = DB::table('settings')->where('id', 1)->first();
  $user_request = DB::table('requestHelp')->where('userid', $user_id)->take(1)->orderBy('id', 'DESC')->get();
  foreach ($user_request as $row) {
      $user_row = DB::table('userdetails')->where('userid', $user_id)->first();
 ...
 <script>
  jQuery(document).ready(function(){
    jQuery("#GetHelpTimer<?php echo $row->id; ?>").jCountdown({
      ...
      displayLabel:false,
      onFinish:function(){
         document.getElementById("GetHelpTimer<?php echo $row->id; ?>").innerHTML = "Merging soon";
      }
    });
  });
</script>
...
function ReceiverView($user_id){
  $settings = DB::table('settings')->where('id', 1)->first();
  $users = DB::table('marching')->where('receiver_id', $user_id)->take(3)->orderBy('id', 'DESC')->get();
                     ...
                     <p>Request Help</p>
                      <form method="POST" action="">
                        <div class="form-group">
                          ...
                          <input type="text" name="amount" readonly value="'.$ids->profit.'" placeholder="Amount to withdraw" class="form-control">
                        </div>
   ...
   echo '<div class="col-md-12">
            ...
            <div class="col-md-6">
                    ...
                    <div class="card-header d-flex align-items-center" style="margin-top: -40px;">
                      <h1 class="h4">My Guider</h1>
                    </div>
                    <div class="card-body text-center">
                     <div class="form-group">
                          <label class="form-control-label"><h1>Parent Guider | '.$Henry->username.' | '.$parents->phonenumber.'  </h1></label>
                          </div>
             ...
             <div class="col-md-6">
                    ...
                    <div class="card-header d-flex align-items-center" style="margin-top: -40px;">
                      <h3 class="h3">My Referral Link</h3>
                    </div>'; ?>
                    ...
                    <div class="form-group">
                          ...
                          if (isset($user) && $settings->activationFee ==1) {
                            ?>
