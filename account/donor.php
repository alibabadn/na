...
?>
     <section class="dashboard-counts no-padding-bottom">
            <div class="container-fluid">
        <div class="row">
            <?php     $ActivationFees = DB::table('activationFee')->where('sender_id', $user_id)->first();
            if($ActivationFees)
            {
            if ($ActivationFees->payment_status =='confirm')
            {
            ?>
       <?php  packagesView(); ?>
            <?php }else
                {
                    echo'<div class="alert alert-danger" role="alert">
         You have not paid your activation fee yet
</div>';
                }
                }
